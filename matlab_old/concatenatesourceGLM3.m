function concatenatesourcePrePstGLM3(subject,band,bandname)
...
if nargin>1,
  ...
  ok   = zeros(length(d),1);
  for k = 1:length(d)
    ok(k) = sum(band==str2num(d(k).name(end-10:end-8)));
  end
  d = d(ok==1);
end
...
for k = 1:length(d)
  ...
  stat13.freq  = str2num(fname(end-10:end-8));
  stat42.freq  = str2num(fname(end-10:end-8));
  stat13b.freq = str2num(fname(end-10:end-8));
  stat42b.freq = str2num(fname(end-10:end-8));
  s13{k} = stat13;
  s42{k} = stat42;
  s13b{k} = stat13b;
  s42b{k} = stat42b;
...
cd('4DprepstGLM3');
fname = [subject.name,'stat4DprepstGLM3',bandname];
