...
for m = 1:length(freqno)
  ...
  if subjno(k)>3
   [stat13,stat42,stat13b,stat42b] = doSourceanalysisDICSprepstResplock(subject,frequency,1);
  else
   [stat13,stat42,stat13b,stat42b] = doSourceanalysisDICSprepstResplock(subject,frequency);
  end
  warning off;
  ...
  save([subject.name,'stat',num2str(frequency,'%03d'),'prepstGLMresplock.mat'],'stat13','stat42','stat13b','stat42b');
