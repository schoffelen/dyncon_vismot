...
for m = 1:length(freqno)
  ...
  [statlc, statli, statrc, statri] = doSourceanalysisDICSpowPrePrevious(subject, frequency);
  ...
  cd([datadir,'source/']);
  save([subject.name,'stat',num2str(frequency,'%03d'),'powPrePreviousCong.mat'],'statlc','statli','statrc','statri');
