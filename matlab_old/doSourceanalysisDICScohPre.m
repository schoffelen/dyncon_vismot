function [statlc, statli, statrc, statri] = doSourceanalysisDICScohPre(subject, frequency)
...
for k = 1:4
  ...
  tmpx1(grid.inside, :) = reshape(scoh1.cohspctrm, numel(scoh1.cohspctrm)/nref, nref);
  tmpx2(grid.inside, :) = reshape(scoh2.cohspctrm, numel(scoh2.cohspctrm)/nref, nref);
  nref  = numel(sel2);
  tmpx3 = zeros(prod(dim), nref);
  tmpx4 = zeros(prod(dim), nref);
  tmpx3(grid.inside, :) = reshape(scoh3.cohspctrm, numel(scoh3.cohspctrm)/nref, nref);
  tmpx4(grid.inside, :) = reshape(scoh4.cohspctrm, numel(scoh4.cohspctrm)/nref, nref);
