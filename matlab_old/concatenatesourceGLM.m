...
if nargin>1,
  ...
  ok   = zeros(length(d),1);
  for k = 1:length(d)
    ok(k) = sum(band==str2num(d(k).name(end-9:end-7)));
  end
  d = d(ok==1);
end
...
for k = 1:length(d)
  ...
  stat13b.freq = str2num(fname(end-9:end-7));
  stat42b.freq = str2num(fname(end-9:end-7));
  s13{k} = stat13;
  s42{k} = stat42;
  s13b{k} = stat13b;
  s42b{k} = stat42b;
...
cd('4DprepstGLMnew');
fname = [subject.name,'stat4DprepstGLMnew',bandname];
