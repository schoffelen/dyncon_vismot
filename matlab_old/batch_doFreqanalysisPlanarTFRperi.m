%function [out] = batch_doFreqanalysisPlanarTFRperi(subjno)
...
for k = 1:numel(subjno)
  ...
  [allfreq] = doFreqanalysisPlanarTFRperi(subject,frequency,smoothing,twindow,toi);
  %cd(subject.pathname);
  %cd('freq');
  cd /analyse/1/Project0002/tmpProject0030
  %save([subject.name,'tfrperiHanning'],'allfreq');
  save([subject.name,'tfrperiHanning2'],'allfreq');
  clear allfreq;
end
out = 1;
%frequency = 20;
