function [statl, statr] = doSourceanalysisDICSpowPrePrevious6(subject, frequency)
...
if strcmp(subject.datafile(1),'h'),
  ...
  vol       = ft_transform_vol(inv(M), vol);
end
...
cfg.keepmom     = 'yes';
source          = ft_sourceanalysis(cfg, freq);
source          = ft_checkdata(source, 'sourcerepresentation', 'new');
...
trlx = freq.trialinfo(:,2);
ix1  = ismember(subject.trlid(:,2:end),trlx) & ismember(subject.trl(:,1:3),[1]) & ...
       subject.correct(:,2:end) & subject.correct(:,1:3);
ix1  = subject.trlid([zeros(nxx,1)==1 ix1]);
indx1a  = find(ismember(trlx, ix1));
ix2  = ismember(subject.trlid(:,2:end),trlx) & ismember(subject.trl(:,1:3),[3]) & ...
       subject.correct(:,2:end) & subject.correct(:,1:3);
ix2  = subject.trlid([zeros(nxx,1)==1 ix2]);
indx2a  = find(ismember(trlx, ix2));

trlx = freq.trialinfo(:,2);
ix1  = ismember(subject.trlid(:,2:end),trlx) & ismember(subject.trl(:,1:3),[4]) & ...
       subject.correct(:,2:end) & subject.correct(:,1:3);
ix1  = subject.trlid([zeros(nxx,1)==1 ix1]);
indx1b  = find(ismember(trlx, ix1));
ix2  = ismember(subject.trlid(:,2:end),trlx) & ismember(subject.trl(:,1:3),[2]) & ...
       subject.correct(:,2:end) & subject.correct(:,1:3);
ix2  = subject.trlid([zeros(nxx,1)==1 ix2]);
indx2b  = find(ismember(trlx, ix2));

sd.cfg.trl = zeros(numel(sd.cumtapcnt),3);

for k = 1:2
  if k==1,
    indx1 = indx1a; %previous lc
    indx2 = indx2a; %previous li
  else
    indx1 = indx1b; %previous rc
    indx2 = indx2b; %previous ri
  end
