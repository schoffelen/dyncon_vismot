function [statlc, statli, statrc, statri] = doSourceanalysisDICSpowPre(subject, frequency)
...
for k = 1:4
  ...
  tmpsd1 = checkdata(selectdata(tmpsd, 'rpt', indx1), 'sourcerepresentation', 'new', 'haspow', 'yes');
  tmpsd2 = checkdata(selectdata(tmpsd, 'rpt', indx2), 'sourcerepresentation', 'new', 'haspow', 'yes');
  dat{1} = tmpsd1; n1 = numel(dat{1}.cumtapcnt);
  dat{2} = tmpsd2; n2 = numel(dat{2}.cumtapcnt);
  dat    = selectdata(dat{:}, 'param', 'pow');
