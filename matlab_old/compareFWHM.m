function [stat13, stat42, stat13b, stat42b] = compareFWHM(subject, frequency, flag)
...
if flag<2,
  load([subject.pathname,'grid/',subject.name,'grid3mm.mat']);
elseif flag==2,
  load([subject.pathname,'grid/',subject.name,'grid3mmNew.mat']);
end
eval('newgrid = grid;');
...
%downsample the grid-size to 6mm
dum = zeros(newgrid.dim);
dum(2:2:end,2:2:end,2:2:end) = 2;
dum(source.inside) = dum(source.inside)+1;
newdim = floor(source.dim/2);
sel    = find(dum>1);
dum    = reshape(dum(sel),newdim);
newinside = find(dum==3);
newoutside = find(dum==2);
source2 = source;
source2.pos = source.pos(sel,:);
source2.xgrid = source.xgrid(2:2:end);
source2.ygrid = source.ygrid(2:2:end);
source2.zgrid = source.zgrid(2:2:end);
source2.inside  = newinside;
source2.outside = newoutside;
source2.dim     = newdim;
source2.leadfield = source.leadfield(sel);
source2.avg.filter = source.avg.filter(sel);
sd   = estimate_fwhm_tetra(source);
sd2  = estimate_fwhm_tetra(source2);
rough  = sd.rough(2:2:end,2:2:end,2:2:end);
rough2 = sd2.rough;
fwhm  = sd.fwhm(2:2:end,2:2:end,2:2:end);
fwhm2 = sd2.fwhm;
