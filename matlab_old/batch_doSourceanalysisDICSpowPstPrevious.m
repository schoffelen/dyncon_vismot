...
for m = 1:length(freqno)
  ...
  [statlc, statli, statrc, statri] = doSourceanalysisDICSpowPstPrevious(subject, frequency);
  ...
  cd([datadir,'source/']);
  save([subject.name,'stat',num2str(frequency,'%03d'),'powPstPreviousCong.mat'],'statlc','statli','statrc','statri');
