function [sc, si, inside] = doSourcegrandavgCong2Previous(foi, band)
...
for k = 1:length(d)
  if ~isempty(strfind(d(k).name,'powPrePreviousCong5')) && isempty(strfind(d(k).name,'stat')) && isempty(strfind(d(k).name,'grandavg')),
    fname = d(k).name;
    fprintf('loading %s\n', fname);
    load(fname);
    s{1}.avg.pow = s{1}.stat; s{1} = rmfield(s{1}, 'stat'); s{1} = rmfield(s{1}, 'dimord');
    s{2}.avg.pow = s{2}.stat; s{2} = rmfield(s{2}, 'stat'); s{2} = rmfield(s{2}, 'dimord');
...
save(['grandavgPrePreviousCong5',band], 'sc', 'si', 'inside', 'names');
