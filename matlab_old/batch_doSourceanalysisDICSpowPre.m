...
for m = 1:length(freqno)
  ...
  [statlc, statli, statrc, statri] = doSourceanalysisDICSpowPre(subject, frequency);
  ...
  cd([datadir,'source/']);
  save([subject.name,'stat',num2str(frequency,'%03d'),'powPreCongClean.mat'],'statlc','statli','statrc','statri');
