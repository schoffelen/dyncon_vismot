function [stat13, stat42, stat13b, stat42b] = doSourceanalysisDICSprepstResplock(subject, frequency, flag)

%Try to do the something similar as doSourceanalysisDICSprepst3 but now with response locked data
%
...
if flag==0 && frequency<37,
  load([subject.name,'mtmfft004resplock']);
elseif flag==0,
  load([subject.name,'mtmfft004resplock']);
  ...
  load([subject.name,'mtmfft012resplock']);
  for k = 1:numel(freqpre)
    freqpre{k}.cfg.trl = trl1{k};
    freqpst{k}.cfg.trl = trl2{k};
  end
elseif flag==1 && frequency<37,
  load([subject.name,'mtmfft_aligned004resplock']);
...
elseif flag==1,
  ...
  load([subject.name,'mtmfft_aligned012resplock']);
  for k = 1:numel(freqpre)
    freqpre{k}.cfg.trl = trl1{k};
    freqpst{k}.cfg.trl = trl2{k};
  end
...
freq{1} = selectdata(freqpre{1:ncnd}, 'param', 'fourierspctrm');
freq{2} = selectdata(freqpst{1:ncnd}, 'param', 'fourierspctrm');
...
cfg2.design(2,:) = freq.rt([btrl(6):etrl(6) btrl(8):etrl(8)])';
...
cfg2.design      = [ones(1,n1) -ones(1,n2)]; %congruency regressor
cfg2.design(2,:) = freq.rt([btrl(9):etrl(9) btrl(7):etrl(7)])';
cfg2.glm.contrast  = [1 0];
...
cfg2.design      = [ones(1,n1) -ones(1,n2)]; %congruency regressor
cfg2.design(2,:) = freq.rt([btrl(1):etrl(1) btrl(3):etrl(3)])';
cfg2.glm.contrast  = [1 0];
...
cfg2.design      = [ones(1,n1) -ones(1,n2)]; %congruency regressor
cfg2.design(2,:) = freq.rt([btrl(4):etrl(4) btrl(2):etrl(2)])';
cfg2.glm.contrast  = [1 0];
