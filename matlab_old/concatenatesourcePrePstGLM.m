...
if nargin>1,
  ...
  ok   = zeros(length(d),1);
  for k = 1:length(d)
    ok(k) = sum(band==str2num(d(k).name(end-15:end-13)));
  end
  d = d(ok==1);
end
...
for k = 1:length(d)
  ...
  stat13.freq  = str2num(fname(end-15:end-13));
  stat42.freq  = str2num(fname(end-15:end-13));
  stat13b.freq = str2num(fname(end-15:end-13));
  stat42b.freq = str2num(fname(end-15:end-13));
  s13{k} = stat13;
  s42{k} = stat42;
  s13b{k} = stat13b;
  s42b{k} = stat42b;
...
cd('4DprepstGLM');
if nargin>1,
  fname = [subject.name,'stat4DprepstGLM_',bandname];
else
