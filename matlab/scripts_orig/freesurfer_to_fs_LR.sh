...
CaretAtlasDirectory=/home/language/jansch/caret/freesurfer_to_fs_LR/standard_mesh_atlases
FreeSurferInstallationDirectory=/opt/freesurfer-v5.1
OutputRootDir=/home/language/jansch/public/fs_tmp
...
for Hemisphere in L R ; do
  ...
  caret_command -surface-generate-inflated $OutputRootDir/$Subject/$InitialMeshDirectory/$Subject.$Hemisphere.midthickness_mni.initial_mesh.coord.gii $OutputRootDir/$Subject/$InitialMeshDirectory/$Subject.$Hemisphere.initial_mesh.topo.gii -iterations-scale 2.5 -generate-inflated -generate-very-inflated -generate-ellipsoid -output-spec $OutputRootDir/$Subject/$InitialMeshDirectory/$Subject.$Hemisphere.mni+orig.initial_mesh.spec -output-inflated-file-name $Subject.$Hemisphere.inflated.initial_mesh.coord.gii -output-very-inflated-file-name $Subject.$Hemisphere.very_inflated.initial_mesh.coord.gii -output-ellipsoid-file-name $Subject.$Hemisphere.ellipsoid.initial_mesh.coord.gii
  mv $Subject.$Hemisphere.inflated.initial_mesh.coord.gii $Subject.$Hemisphere.very_inflated.initial_mesh.coord.gii $Subject.$Hemisphere.ellipsoid.initial_mesh.coord.gii $OutputRootDir/$Subject/$InitialMeshDirectory/
  ...
  for Surface in inflated very_inflated ellipsoid; do
