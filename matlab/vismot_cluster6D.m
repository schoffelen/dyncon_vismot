function [clus] = vismot_cluster6D(cfg, dat)
...
if cfg.tail==0
  ...
  if length(cfg.threshold)==1
    ...
    if strcmp(cfg.method, 'fixed')
      ...
      clus1            = vismot_cluster6D(tmpcfg, tmpdat);
      tmpcfg.tail      = -1;
      tmpcfg.threshold = -t;
      clus2            = vismot_cluster6D(tmpcfg, tmpdat);
    ...
    if nvox>0
      ...
      if mode(val)==clusind(1)
        if all(val==val(1)) && sum(val==val(1))==nvox
          ...
          if hasindx, clus(cnt).hit    = -1-double(sum(ismember(clus(cnt).vox, indx))==2); end
      ...
      elseif all(val==val(1)) && sum(val==val(1))==nvox,
        ...
        clus(cnt).vox    = int32(unique([indvec1;indvec2]));
        clus(cnt).nvox   = length(clus(cnt).vox);
        if hasindx, clus(cnt).hit    = double(sum(ismember(clus(cnt).vox, indx))==2); end
        clus(cnt).thr    = threshold;
      else
...
if ~isempty(cfg.indx)
  ...
  [ix(:,1), ix(:,2), ix(:,3)] = ind2sub(dim, cfg.indx);
