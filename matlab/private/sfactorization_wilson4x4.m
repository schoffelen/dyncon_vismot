function [H, Z, S, psi] = sfactorization_wilson4x4(S,freq,Niterations,tol,cmbindx,fb,init,checkflag)

% SFACTORIZATION_WILSON4X4 performs quartetwise non-parametric spectral factorization on
...
if nargin<5, 
  error('FieldTrip:connectivity:sfactorization_wilson4x4', 'when requesting multiple pairwise spectral decomposition, ''cmbindx'' needs to be specified');
end
...
if ~all(dfreq==dfreq(1))
  error('FieldTrip:connectivity:sfactorization_wilson4x4', 'frequency axis is not evenly spaced');
end

if freq(1)~=0
  warning_once('FieldTrip:connectivity:sfactorization_wilson4x4', 'when performing non-parametric spectral factorization, the frequency axis should ideally start at 0, zero padding the spectral density'); 
  dfreq = mean(dfreq);
  npad  = freq(1)./dfreq;
...
Sarr(4,1,N1:N2,:) = conj(Sarr(4,1,N1:N2,:));
Sarr(1,4,N1:N2,:) = conj(Sarr(1,4,N1:N2,:));
Sarr(2,3,N1:N2,:) = conj(Sarr(2,3,N1:N2,:));
Sarr(3,2,N1:N2,:) = conj(Sarr(3,2,N1:N2,:));
Sarr(2,4,N1:N2,:) = conj(Sarr(2,4,N1:N2,:));
Sarr(4,2,N1:N2,:) = conj(Sarr(4,2,N1:N2,:));
Sarr(3,4,N1:N2,:) = conj(Sarr(3,4,N1:N2,:));
Sarr(4,3,N1:N2,:) = conj(Sarr(4,3,N1:N2,:));
...
gam = real(reshape(ifft(reshape(Sarr, [16*m N2]), [], 2),[4 4 m N2]));
  ...
  switch init
    case 'chol'
      ...
      if dum
        ...
        tmp = rand(4,4); %arbitrary initial condition
        tmp = triu(tmp);
      end
    case 'rand'
      tmp = rand(4,4); %arbitrary initial condition
...
for iter = 1:Niterations
  ...
  invpsi = inv4x4(psi);
  %invpsi = pinv4x4(psi,0.0001);
  g      = sandwich4x4(invpsi, Sarr) + I;
  gp     = PlusOperator4x4(g,m,N); %gp constitutes positive and half of zero lags 
  
  psi_old = psi;
  psi     = mtimes4x4(psi, gp);
...
gamtmp = reshape(real(ifft(transpose(reshape(psi, [16*m N2]))))', [4 4 m N2]);
...
H = complex(zeros(4,4,m,N));
S = complex(zeros(4,4,m,N));
for k = 1:N
  for kk = 1:m
...
siz = [size(H) 1 1];
H   = reshape(H, [16*siz(3) siz(4:end)]);
siz = [size(S) 1 1];
S   = reshape(S, [16*siz(3) siz(4:end)]);
siz = [size(Z) 1 1];
Z   = reshape(Z, [16*siz(3) siz(4:end)]);
siz = [size(psi) 1 1];
psi = reshape(psi, [16*siz(3) siz(4:end)]);
...
%end
indx = [2 3 4 7 8 12]'*ones(1,ncmb) + ones(6,1)*(0:16:(ncmb-1)*16);
%beta0(2:4:4*ncmb)   = 0;
beta0(indx(:))      = 0;
gamp(1,:)           = beta0;
gamp(nfreq+1:end,:) = 0;
...
% reconstituting
gp = fft(gamp);
gp = reshape(transpose(gp), [4 4 ncmb numel(gp)/(16*ncmb)]); 
