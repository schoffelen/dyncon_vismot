classdef vismot_circularGraph < handle
    ...
    properties
        Node = circularGraphNode(0,0); % Array of nodes
        ColorMap;         % Colormap
    ...
    methods
        function this = vismot_circularGraph(adjacencyMatrix,varargin)
            % Constructor
            p = inputParser;
            ...
            addParameter(p, 'std', []);
            addParameter(p, 'maxnumber', []);
            addParameter(p, 'maxnumberstd', []);
            
            parse(p,adjacencyMatrix,varargin{:});
            this.ColorMap = p.Results.ColorMap;
            this.Label    = p.Results.Label;
            minLineWidth  = p.Results.minLineWidth;
            standdev      = p.Results.std;
            V             = p.Results.maxnumber;
            Vstd          = p.Results.maxnumberstd;
            ...
            for i = 1:length(adjacencyMatrix)
                this.Node(i) = circularGraphNode(cos(t(i)),sin(t(i)));
                this.Node(i).Color = [0 0 0];%this.ColorMap(i,:);
            ...
            lineWidth = lineWidthCoef*lineWidth + sign(lineWidth).*minLineWidth;
            %       end
            
            if ~isempty(standdev)
                [row2,col2,v2] = find(standdev);
                if ~isempty('Vstd')
                    lineWidth2 = v2./Vstd;
                else
                    lineWidth2 = v2./max(v2);
                end
                lineWidth2 = 4*lineWidthCoef*lineWidth2 + abs(lineWidth);
            end
            ...
            for i = 1:length(v)
                if row(i) ~= col(i)
                    if lineWidth(i)<0
                        clrmap = this.ColorMap(row(i),:, 2);
%                         LineStyle = '--';
                    else
%                         LineStyle = '-';
                        clrmap = this.ColorMap(row(i),:, 1);
                    end
                    LineStyle = '-';
