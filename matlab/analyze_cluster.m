...
elseif length(clus)==1
  ...
  [ix(:,1),ix(:,2),ix(:,3),ix(:,4),ix(:,5),ix(:,6)] = ...
       ind2sub([dim dim], clus.vox);
  
  dpos  = ix - repmat([targetindx(1,:) targetindx(2,:)], [size(ix,1) 1]);
  mind  = sqrt(sum(dpos.^2,2));
  dpos  = ix - repmat([targetindx(2,:) targetindx(1,:)], [size(ix,1) 1]);
